...
class k_class:
    def __init__(self, config, debug=False):
        ...
        num_classes = self.config.data_loader.k_classes
    ...
    def extract_data_from_patch(self, patch, phase=None):
        ...
        if phase=='Test':
            ...
            gt = patch['distance'][tio.DATA].long().cuda()
            reshaped = gt.squeeze(1)
            gt = F.one_hot(reshaped, num_classes=25).permute(0, 4, 1, 2, 3).float()
    ...
    def predict(self, path_origin):
        ...
        with torch.inference_mode():
            ...
            subject_dict = {
                ...
                'distance': tio.LabelMap(path_origin),
            }
            ...
            f = torch.zeros(size=output[0].size())
            o = output>0.5
