...
from skimage import segmentation as skimage_seg #addboundary
import numpy as np
...
def bin2dist(mask):
    dist_transform = -1 * distance_transform_edt(mask.data[0].bool(), sampling=[abs(mask.affine[0][0]), abs(mask.affine[1][1]), abs(mask.affine[2][2])])
    dist_transform_inv = distance_transform_edt(~mask.data[0].bool(), sampling=[abs(mask.affine[0][0]), abs(mask.affine[1][1]), abs(mask.affine[2][2])])
...
class Approximated_Heaviside(torch.nn.Module):
    def __init__(self, slope=1500.0):
        super(Approximated_Heaviside, self).__init__()
        self.slope = slope

    def forward(self, x):
        return torch.special.expit(self.slope * x)
